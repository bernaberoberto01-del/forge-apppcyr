...
export default function Layout({ session }) {
  ...
  return (
    <div className="flex min-h-screen bg-[#F5F5F0]">
      ...
      <aside className="w-60 bg-[#111] flex flex-col fixed h-full z-20 hidden md:flex">
            ...
            <NavLink key={item.to} to={item.to}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${
                  isActive
                    ? 'bg-[#FF5C00] text-white font-semibold shadow-sm'
        ...
        <div className="p-3 border-t border-white/8">
          <div className="flex items-center gap-3 px-2 py-2 mb-1">
            <div className="w-8 h-8 bg-[#FF5C00]/20 rounded-full flex items-center justify-center text-[#FF5C00] font-bold text-sm flex-shrink-0">
              {inicial}
            </div>
      ...
      {/* Nav móvil */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-[#111] border-t border-white/8 flex z-20 safe-area-bottom">
        {nav.map(item => (
          <NavLink key={item.to} to={item.to}
            className={({ isActive }) =>
              `flex-1 flex flex-col items-center py-3 transition-all ${
                isActive ? 'text-[#FF5C00]' : 'text-white/40'
              }`}>
