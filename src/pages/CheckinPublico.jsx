...
export default function CheckinPublico() {
    ...
    return (
      <button type="button" onClick={() => set(field, val)}
        className={`w-10 h-10 rounded-xl text-sm font-semibold transition-all ${active
          ? (red && val >= 4 ? 'bg-red-500 text-white shadow-md' : active && val <= 3 && red ? 'bg-amber-400 text-white' : 'bg-orange-500 text-white shadow-md')
  ...
  if (notFound) return <div className="min-h-screen flex items-center justify-center"><div className="text-center"><p className="text-3xl mb-2">🔗</p><p className="text-gray-500">Enlace no válido</p></div></div>
  if (enviado) return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-[#F7F7F7]">
      <div className="text-center max-w-sm">
        ...
        <p className="text-gray-500 text-sm">Tu entrenador ya tiene tu seguimiento semanal. ¡Sigue así! 💪</p>
      </div>
    </div>
  )
    ...
    <div className="min-h-screen bg-[#F7F7F7]">
      <div className="bg-[#111] px-4 py-4 sticky top-0 z-10">
        <div className="max-w-lg mx-auto flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-gray-400 text-xs">Hola {cliente.nombre.split(' ')[0]}, ¿cómo ha ido tu semana?</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto p-4 pb-10 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" step="0.1" value={form.peso} onChange={e => set('peso', e.target.value)}
                className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-orange-500" placeholder="70.5" />
            </div>
            <div>
              ...
              <input type="number" value={form.pasos_diarios} onChange={e => set('pasos_diarios', e.target.value)}
                className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-orange-500" placeholder="8000" />
            </div>
          </div>
          <div className="mt-3">
            <label className="text-xs font-medium text-gray-600 mb-2 block">Sesiones completadas esta semana: <span className="text-orange-500 font-bold">{form.sesiones_semana}</span></label>
            <div className="flex gap-2">
              {[0,1,2,3,4,5,6,7].map(v => (
                <button key={v} type="button" onClick={() => set('sesiones_semana', v)}
                  className={`w-9 h-9 rounded-xl text-sm font-semibold transition-all ${form.sesiones_semana === v ? 'bg-orange-500 text-white' : 'border border-gray-200 text-gray-500'}`}>
                  {v}
                </button>
              ))}
            </div>
        ...
        {ESCALAS.map(({ label, field, min, max, suffix, red, desc }) => (
          <div key={field} className="bg-white rounded-2xl border border-gray-100 p-4">
            <div className="flex items-center justify-between mb-1">
              <p className="text-sm font-semibold text-[#111]">{label}</p>
              <span className={`text-lg font-bold ${red && form[field] >= 4 ? 'text-red-500' : 'text-orange-500'}`}>{form[field]}{suffix}</span>
            </div>
            {desc && <div className="flex justify-between mb-3"><span className="text-xs text-gray-400">{desc[0]}</span><span className="text-xs text-gray-400">{desc[1]}</span></div>}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-4">
          ...
          <textarea value={form.comentario} onChange={e => set('comentario', e.target.value)} rows={3}
            className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-orange-500 resize-none"
            placeholder="¿Algo que quieras contarme esta semana?" />
        </div>

        <button onClick={enviar} disabled={sending}
          className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-4 rounded-2xl transition-colors disabled:opacity-50 text-base">
          {sending ? 'Enviando...' : 'Enviar seguimiento 💪'}
        </button>

        <p className="text-center text-xs text-gray-400">Forge Studio OS · Tus datos están protegidos</p>
      </div>
    </div>
  )
}
