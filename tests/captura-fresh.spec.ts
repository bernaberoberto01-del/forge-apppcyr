...
test('captura fresh sin caché', async ({ browser }) => {
  // Contexto nuevo sin caché
  ...
  await page.screenshot({ path: '/tmp/fresh-inicio.png', fullPage: true })

  // Rutina
  await page.click('button:has-text("Rutina")')
  await page.waitForTimeout(2000)
  await page.screenshot({ path: '/tmp/fresh-rutina.png', fullPage: true })

  // Nutrición
  await page.click('button:has-text("Nutrición")')
  await page.waitForTimeout(2000)
  await page.screenshot({ path: '/tmp/fresh-nutricion.png', fullPage: true })
