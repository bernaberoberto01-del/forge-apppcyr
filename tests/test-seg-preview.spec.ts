...
// URL del deploy preview más reciente
const BASE='https://forge-studio-7j3jbkd81-bernaberoberto01-4397s-projects.vercel.app'
const KEY='sb-qdpqpbkppkhzcxpfypvf-auth-token'

test('seguimiento preview', async ({ browser }) => {
  const ctx = await browser.newContext({ viewport: { width: 390, height: 844 } })
  const page = await ctx.newPage()
  const res = await page.request.post(`${SUPA}/auth/v1/token?grant_type=password`,
    { headers:{'apikey':ANON,'Content-Type':'application/json'}, data:{email:'bernaberoberto01@gmail.com',password:'Roberto72'} })
  ...
  await page.screenshot({path:'/tmp/seg-preview.png', fullPage:true})
